---
title: Apache Iggy 0.9.0 Release
author: piotr
tags: ["release", "0.9.0", "announcement", "clustering", "vsr", "benchmarks"]
date: 2026-09-21
---
## Release 0.9.0

Five months after [0.8.0](https://iggy.apache.org/blogs/2026/04/22/release-0.8.0/), we're proud to release **[Apache Iggy 0.9.0](https://github.com/apache/iggy/releases/tag/server-0.9.0)**. It is our first release as an **Apache Top-Level Project**, following the [graduation from the Incubator](https://iggy.apache.org/blogs/2026/08/24/apache-iggy-top-level-project-tlp-graduation/) on August 19, 2026, and the first one without the `-incubating` suffix. It is also the release we have been working towards for the past year.
...
Then came the correctness fixes found by the simulator, the crash tests and real cluster runs: **[view change no longer discards committed ops](https://github.com/apache/iggy/pull/3824)**, **[a replica is kept off a hole in its committed prefix](https://github.com/apache/iggy/pull/4073)**, **[repaired prepares can't rewind the WAL parent](https://github.com/apache/iggy/pull/3978)**, **[the dedup fence survives capacity eviction](https://github.com/apache/iggy/pull/3934)**, **[writes recover after a fast primary rejoin](https://github.com/apache/iggy/pull/3987)**, **[a late partition materialiser can't wedge its group](https://github.com/apache/iggy/pull/4007)**, **[partition and metadata repair are driven from the tick](https://github.com/apache/iggy/pull/4006)**, **[an unservable header counts as a nack](https://github.com/apache/iggy/pull/4109)**, **[frames no plane claims are refused](https://github.com/apache/iggy/pull/4103)**, **[torn segment tails are truncated instead of resurrected](https://github.com/apache/iggy/pull/3946)**, and, as the last change before the tag, **[spurious heartbeat elections are prevented](https://github.com/apache/iggy/pull/4194)**.
...
The details are in [Durability](/docs/server/durability) and [Cluster Durability](/docs/clustering/durability).
...
See [Cluster Security](/docs/clustering/security).
...
Every node loads the same configuration and is told apart only by `--replica-id`:
...
Every roster field can also be set through typed `IGGY_CLUSTER_*` environment variables, which is what the new **[Helm chart cluster mode](https://github.com/apache/iggy/pull/4035)** renders. The **[Deploy a Cluster](/docs/clustering/deploy)** guide walks through the whole setup, and the **[Configuration](/docs/clustering/configuration)** reference covers every `[cluster]` key, including consensus timing (`heartbeat_timeout`, view change and repair intervals, all on a fixed 10ms tick).
...
The [Viewstamped Replication](/docs/clustering/vsr) page is the entry point to all of this.
...
We're not done. A dedicated performance effort started right after the 0.9.0 code freeze, and the first results are in. The tables below compare **the 0.9.0 release against the current development branch**, both measured with `iggy-bench` on the same host with identical workload parameters. Each cell reads `0.9.0 -> next`. Latencies are in milliseconds.

A short legend. *Pinned producer* and *pinned consumer* are the `iggy-bench` kinds where every actor owns its own stream and partition. *Actors* is the number of concurrent producers or consumers. Rate-limited rows hold the aggregate throughput at 800 MB/s or 500 MB/s, so the latency columns compare like with like. *No rate limit* rows push as hard as the client can. *Persisted* rows use topics created with `durability=persisted`, so every acknowledgement waits for stable storage at the quorum. *Cold* consumers read data that is not cached yet, *warm* consumers re-read data that is.

<Bench090LatencyChart />

**Single node**

| Workload | MB/s | p50 ms | p99 ms | p999 ms |
| --- | --- | --- | --- | --- |
| pinned producer, 20 actors, 800 MB/s | 800.1 -> 800.1 | 0.65 -> 0.588 | 1.313 -> 1.145 | 11.087 -> 2.956 |
...
| pinned consumer, 20 actors, warm, 800 MB/s | 800.1 -> 800.1 | 0.521 -> 0.544 | 1.022 -> 0.923 | 1.573 -> 3.92 |
| pinned producer, 1 actor, 500 MB/s | 500 -> 500 | 0.347 -> 0.352 | 0.679 -> 0.683 | 1.356 -> 1.046 |
| pinned consumer, 1 actor, warm, 500 MB/s | 500 -> 500 | 0.272 -> 0.296 | 0.468 -> 0.478 | 0.491 -> 0.502 |
| pinned producer, 20 actors, no rate limit | 2062.1 -> 2096.5 | 1.086 -> 0.792 | 26.597 -> 46.431 | 40.869 -> 78.082 |
| pinned consumer, 20 actors, no rate limit | 2713.4 -> 2532.5 | 0.679 -> 0.619 | 6.151 -> 5.869 | 6.476 -> 23.418 |
...
| pinned producer, 1 actor, persisted, 500 MB/s | 234.4 -> 233.6 | 1.047 -> 1.062 | 1.2 -> 1.223 | 10.857 -> 1.759 |
...
| pinned producer, 20 actors, 800 MB/s | 800.0 -> 800.0 | 1.782 -> 1.679 | 3.186 -> 3.25 | 3.683 -> 3.66 |
| pinned producer, 20 actors, no rate limit | 1134.5 -> 1132.5 | 4.366 -> 4.357 | 5.159 -> 5.296 | 6.299 -> 5.929 |
| pinned consumer, 20 actors, no rate limit | 2710.1 -> 2594.3 | 0.682 -> 0.657 | 6.12 -> 5.758 | 6.445 -> 23.362 |
| pinned producer, 1 actor, 500 MB/s | 337.2 -> 325.4 | 0.716 -> 0.735 | 0.973 -> 1.016 | 1.057 -> 1.13 |
| pinned consumer, 1 actor, warm, 500 MB/s | 500.0 -> 500.3 | 0.297 -> 0.326 | 0.493 -> 0.536 | 0.533 -> 0.574 |

The tails are where the work shows. On a three-node cluster with `persisted` topics, the **p99 of a 20-producer workload dropped from 7.07 ms to 2.94 ms and the p999 from 12.4 ms to 3.9 ms**. The single-producer p999 on the same cluster went from 10.9 ms to 1.8 ms. On a single node, the 20-producer **p999 fell from 11.1 ms to 3.0 ms**. Cold consumer reads improved on both setups. A replicated, persisted write on a three-node cluster now completes in about 2.2 ms at the median while the cluster ingests 800 MB/s.

Not every row moved in the right direction. The unthrottled rows show higher tails on the development branch, and a few single-actor rows are slightly slower. **These numbers are a snapshot of work in progress, not a final result.** The branch is being profiled and tuned as we write this, and the next version will carry further changes aimed at both the throughput and the tail columns. We're publishing the snapshot anyway, because we'd rather show where the work stands than wait for a perfect table.

What is in the pipeline, in the order we expect it to land:

- **[Deferred polling](https://github.com/apache/iggy/issues/3470)**: a poll carries a wait timeout, and when a partition has nothing new the server holds the request and completes it as soon as messages arrive or the timeout expires, instead of answering empty and making the consumer ask again. Fewer wasted round trips, less CPU churn on both sides, lower end-to-end latency.
- **More concurrency in I/O**: one task per read and per write, so disk and network operations overlap instead of waiting on each other within a shard. The first piece is already in review: **[read-ahead on replica sockets](https://github.com/apache/iggy/pull/4224)**, so one socket read serves a whole burst of replication frames instead of two reads per frame.
...
Docker images are available on **[Docker Hub](https://hub.docker.com/u/apache?page=1&search=iggy)**: `apache/iggy:0.9.0` for the server and `apache/iggy-connect:0.5.0` for the connectors runtime.
