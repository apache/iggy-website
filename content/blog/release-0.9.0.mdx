...
description: Apache Iggy 0.9.0 brings VSR clustering, per-topic durability, SDK and connector updates, and a first look at performance work for the next release.
author: piotr, hubert
...
Both policies write data to disk. On a single node, the quorum is one: *replicated* provides no extra copy, while *persisted* waits for local stable storage. These labels describe producer acknowledgements. Consumer rows measure reads. See [Durability](/docs/server/durability) for the full guarantees.
