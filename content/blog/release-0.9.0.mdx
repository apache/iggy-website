...
Three changes are in flight, and the tables above measure them together against 0.9.0. A fourth is on the drawing board.
...
This one is a design, not code, and the read-ahead has to justify it first. If the link shard stays saturated behind the buffer, more links are the answer. If not, the cheaper fix already won.
...
Beyond these, in the order we expect it to land:
