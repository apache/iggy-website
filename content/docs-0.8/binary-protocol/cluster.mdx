...
Replicas talk to each other with the same 256-byte [framing](/docs/0.8/binary-protocol/framing) the client protocol uses, on a **dedicated TCP port**, with their own set of `command` discriminants. None of these frames ever appears on a client connection, and a client cannot reach this plane: it is a separate listener, gated by a handshake.
...
| 5 | `Request` | client to server | Client command ([framing](/docs/0.8/binary-protocol/framing)) |
