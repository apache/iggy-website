...
The option keys differ per transport (for example QUIC uses `reconnection_max_retries` and `reconnection_reestablish_after` where TCP uses `reconnection_retries` and `reestablish_after`). Unknown keys are rejected as hard errors. See [Connection Strings](/docs/0.8/sdk/connection-strings) for the full per-transport option tables and defaults.
...
The `After` and `IntervalOrAfter` variants *only* work with `consume_messages()` from the `IggyConsumerMessageExt` trait (see [Stream Builder](/docs/0.8/sdk/rust/stream-builder)). The plain `next()` loop cannot observe when your processing finished.
