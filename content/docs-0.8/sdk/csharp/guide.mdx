...
This guide covers client configuration and the full `IIggyClient` API surface - the low-level, per-call operations. For the ergonomic producer/consumer abstractions built on top of these, see the [High-level SDK](/docs/0.8/sdk/csharp/high-level-sdk).

All examples assume you already have a connected, authenticated client (see [Creating a client](/docs/0.8/sdk/csharp/intro#creating-a-client)).
...
To send many payloads without a `byte[]` allocation per message, build them into a single pooled buffer with `RentedMessageBatchBuilder` - see [Publishing with rented batches](/docs/0.8/sdk/csharp/high-level-sdk#publishing-with-rented-batches).
...
The high-level `IggyConsumer` exposes the same pooled path as an async stream via `ReceiveRentedAsync` - see [Consuming with rented buffers](/docs/0.8/sdk/csharp/high-level-sdk#consuming-with-rented-buffers).
