...
These samples use the [High-level SDK](/docs/0.8/sdk/csharp/high-level-sdk) - the recommended way to build producers and consumers. For the low-level, per-call equivalents, see the [Guide](/docs/0.8/sdk/csharp/guide).
...
A typed publisher/consumer pair that (de)serializes a record as JSON. The typed builders are configured via statements rather than one fluent chain - the `With*` methods return the untyped base builder, so `Build()` **must be called on the typed builder variable** (see [Typed consumer](/docs/0.8/sdk/csharp/high-level-sdk#typed-consumer)):
