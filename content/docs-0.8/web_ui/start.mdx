...
Here's the full example of the `docker-compose.yml` file that starts the Iggy server, initializes it with the `my-stream` stream and `my-topic` topic with the help of [Iggy CLI](/docs/0.8/cli/start), and starts the Iggy Web UI:
