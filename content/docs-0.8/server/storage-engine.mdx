...
description: "The segmented append-only log, and how streams, topics, partitions and segments map onto files on disk."
---
...
The full byte-level tables live on the [Message batches](/docs/0.8/binary-protocol/messages) page. Absent at-rest encryption, what lands on disk is exactly what the producer sent, with the server stamping `partition_id` at admission and `base_offset` / `base_timestamp` (plus a `batch_checksum` recompute) at journal append.
...
Two independent retention policies exist, both **per-topic creation options** (see [Topic options](/docs/0.8/server/topic-options)). The segment cleaner enforces them:
...
The config loader rejects the old `[system]` table and `IGGY_SYSTEM_*` environment mappings. The full mapping is in [Relocated configuration keys](/docs/0.8/server/configuration#relocated-configuration-keys).
