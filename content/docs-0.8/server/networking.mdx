---
title: Networking
description: "The four transports Iggy serves simultaneously, TCP, QUIC, WebSocket and HTTP, and what each is best suited to."
---
...
The `[websocket]` section exposes the frame-tuning knobs (`read_buffer_size`, `write_buffer_size`, `max_write_buffer_size`, `max_message_size`, `max_frame_size`, `accept_unmasked_frames`). See [Configuration](/docs/0.8/server/configuration). TLS for WebSocket has its own `[websocket.tls]` section.
