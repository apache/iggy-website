---
title: Benchmarking
description: "How Iggy is benchmarked, the tooling that ships with it, and the public platform for comparing results."
---
...
Use the [Linux tuning guide](/docs/0.8/server/linux-tuning) to check process limits, swap, huge pages, CPU/NUMA placement, writeback, and network capacity before measuring. Record the exact host settings and change one variable at a time. Use a separate load generator, or disjoint CPU sets when it shares a host with the server.
