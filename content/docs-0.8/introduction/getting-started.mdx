...
For real-world applications, you might typically prefer the **[high-level SDK](/docs/0.8/sdk/rust/high-level-sdk)**, which provides `IggyProducer` and `IggyConsumer` builders with auto-batching, consumer groups, offset management, reconnection, and retry logic out of the box. Check the [Stream Builder](/docs/0.8/sdk/rust/stream-builder) docs for the most ergonomic API.

For examples in other programming languages, see the [SDK section](/docs/0.8/sdk/introduction).
...
If you're eager to find out how to build more advanced (and configurable) applications, check the Rust [examples](/docs/0.8/sdk/rust/examples). Nevertheless, let's focus on implementing our producer side :)
...
So far, so good, however, before we will be able to publish any messages to our streaming server, at first, we need to create the stream, topic and partition(s) - if you're unfamiliar with these concepts, please refer to [concepts](/docs/0.8/introduction/concepts) where all of them are described in-depth.
