...
- **[Connectors](/docs/0.8/connectors/introduction)** - sinks, sources and data transformations based on the **custom Rust plugins** loaded dynamically at runtime
- **[Model Context Protocol](/docs/0.8/ai/mcp)** - provide context to LLM with **MCP server** exposing 40+ tools for full Iggy management
...
- Built-in **[CLI](/docs/0.8/cli/start)** to manage the streaming server installable via `cargo install iggy-cli`
- Built-in **[Web UI](/docs/0.8/web_ui/start)** dashboard (Svelte) that can be embedded directly in the server binary or run as a standalone container
...
- Built on **Viewstamped Replication (VSR)** consensus: runs as a single node by default, with multi-node [clustering](/docs/0.8/clustering/vsr) available via the `[cluster]` configuration
