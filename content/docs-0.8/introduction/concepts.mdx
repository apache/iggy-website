...
In the SDKs, each message carries a 64-byte in-memory header (little-endian fields). On the wire and on disk, messages travel inside batch records with a compact 48-byte per-message frame. See the [Binary Protocol](/docs/0.8/binary-protocol) section for the exact encodings.
...
To see how the server schedules these concepts across CPU cores and stores them on disk, head over to [architecture](/docs/0.8/introduction/architecture).
