...
description: How Apache Iggy replicates data across a cluster with VSR
---
...
Reads use the local replicated state where possible. Writes return after the required VSR commit or return a retryable error when the replica is changing view or catching up. See [Client failover](/docs/0.8/clustering/client-failover) for how the SDKs handle both.
...
- [Deploy a cluster](/docs/0.8/clustering/deploy): build the binaries and run a three-node cluster
- [Configuration](/docs/0.8/clustering/configuration): every `[cluster]` setting and its validation rules
- [Security](/docs/0.8/clustering/security): replica authentication, key rotation, and TLS
- [Client failover](/docs/0.8/clustering/client-failover): reads, writes, and leader redirection from the client side
