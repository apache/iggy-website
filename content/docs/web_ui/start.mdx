...
Remember that `PUBLIC_IGGY_API_URL` is consumed by the browser: if the Iggy server also runs in Docker, use the host-published HTTP port here, not the container's network alias.

### Local development

The Web UI lives in the `web/` folder of the [repository](https://github.com/apache/iggy/tree/master/web). Use a Node.js version supported by the frontend toolchain (`^20.19.0 || >=22.12.0` per Vite; 22 LTS is a safe default) and `npm` (`pnpm` and `yarn` are not part of the supported workflow):
...
- **Overview** - server health overview and key metrics
...
Some pages visible in the navigation (Clients, Logs, terminal, server settings) are placeholders that are not functional yet; see the [roadmap](https://github.com/apache/iggy/tree/master/web#roadmap) for what is planned.
