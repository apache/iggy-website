...
This page lists every top-level `iggy` command. Examples assume you are already authenticated, for example via `iggy login`; see [installation and authentication](/docs/cli/start). Run `iggy <command> --help` for the full flag list of any command.
...
| `-m, --message-key <KEY>` | Let the server pick the partition from the key (mutually exclusive with `--partition-id`) |
