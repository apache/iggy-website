...
The Iggy CLI is a command-line interface for managing an Iggy server: streams, topics, partitions, messages, users, tokens and more. It lives in the core repository under the `core/cli` directory. The crate is named `iggy-cli`, while the installed binary is named `iggy`.
...
Once installed, run `iggy` without arguments to print an overview of the available commands, or head to the [command reference](/docs/cli/commands).
...
`-u`, `-t` and `-n` are **mutually exclusive**. Avoid `-p` with the password inline where possible: it lands in your **shell history**. Prefer `iggy login`, the interactive prompt, or the environment variables.
...
`--store-token` is mutually exclusive with an expiry: stored tokens **never expire**. Stored tokens are **namespaced per server address**, so a token stored for one server is not visible when connecting to another. Alternatively, create a token with an expiry and pass its value with `-t`:
...
There is no `--context` flag: the active context is persistent state, switched with `iggy context use <name>`. Flags passed on the command line override the corresponding values from the active context.
