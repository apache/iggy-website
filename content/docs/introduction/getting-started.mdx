...
The completed sample can be found in [repository](https://github.com/apache/iggy/tree/master/examples/rust/src/getting-started), however, we will go through the implementation step by step, so that you can get a better understanding of how to build the applications from scratch.
