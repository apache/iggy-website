...
Alternatively, build from source by cloning the [repository](https://github.com/apache/iggy) and running `cargo r --bin iggy-server`. All the data used by the server will be persisted under the `local_data` directory, unless specified differently in the configuration.
