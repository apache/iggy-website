...
On Linux the server needs kernel 5.19 or newer. macOS has no kernel requirement, but Docker Desktop on 0.9.0 needs one extra setting. Under WSL2, run `wsl --update` first. See [System requirements](/docs/server/introduction#system-requirements) for the details.
