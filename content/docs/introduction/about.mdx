...
- **Embedded** - compiled into the server binary (with `iggy-web` feature flag), served at the `/ui` endpoint when `http.web_ui = true`
- **Standalone** - as a separate container via `docker pull apache/iggy-web-ui:edge`

Features include stream/topic/partition management, a message browser with JSON/string/XML decoders, and user management. The logs and terminal pages are placeholders, and the server settings page has a disabled Save action rather than an operational configuration editor.
