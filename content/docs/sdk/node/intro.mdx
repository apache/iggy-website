...
The SDK speaks the current (VSR) wire protocol exclusively and does not fall back to older formats, so it requires an Iggy server that speaks it. Keep the client and server versions in step: pair the newest package with the newest server.
...
The client constructor accepts a few knobs worth knowing about:
