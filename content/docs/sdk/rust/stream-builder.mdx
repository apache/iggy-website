...
(or with `IGGY_ROOT_USERNAME`/`IGGY_ROOT_PASSWORD` set). Otherwise root gets a generated password and the examples fail with `InvalidCredentials`.
...
Note that the consumer is bound as `mut`: `consume_messages` (and the underlying message stream) requires **mutable access**.
