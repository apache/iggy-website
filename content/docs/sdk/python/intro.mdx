...
Because the wheel bundles the Rust SDK, it speaks the current Iggy wire protocol only and does not fall back to older formats. Keep the client and server versions in step: pair the newest wheel with the newest server.
...
The SDK covers much more than the quick start above:
...
- **Administration**: user and permission management. Personal access tokens can be used for login via `AutoLogin.personal_access_token(...)` in the connection config; PAT management is not exposed yet.
