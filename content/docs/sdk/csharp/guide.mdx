...
Partition selection is resolved **client-side** on both transports; the server never routes. Balanced partitioning is a client-side round-robin over the topic's partitions, and key-based routing hashes the key with xxHash32 modulo the partition count, consistent with the Rust SDK.
