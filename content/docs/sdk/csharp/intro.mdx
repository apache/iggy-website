...
The SDK targets .NET 8 and .NET 10 (`net8.0` and `net10.0`). It currently ships as **prerelease versions** (`0.9.0-edge.x` at the time of writing), so pass `--prerelease` to get the version these docs describe.
