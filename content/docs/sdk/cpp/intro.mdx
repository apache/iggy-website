...
The Iggy C++ SDK wraps the Rust SDK through a [CXX](https://cxx.rs) FFI bridge, which means it can access all transport protocols (TCP, QUIC, HTTP, WebSocket) through connection strings. The source code is available at [github.com/apache/iggy/tree/master/foreign/cpp](https://github.com/apache/iggy/tree/master/foreign/cpp).

This SDK is not published to any package registry. It is consumed from the monorepo and built from source with Bazel.

## Building from source

The project builds with Bazel (`BUILD.bazel` / `MODULE.bazel`). The Bazel build drives the system-provided cargo toolchain to compile the Rust bridge, so concurrent runs can race; run builds serially.
