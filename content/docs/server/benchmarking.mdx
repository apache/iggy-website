...
Iggy comes with a built-in benchmarking tool `iggy-bench` that measures the performance of the server. It is written in Rust and uses the `tokio` runtime for asynchronous I/O, mimicking the example client applications, so you can use it to estimate the performance of the server in your environment. It is part of the [core repository](https://github.com/apache/iggy/tree/master/core/bench) and lives in the `core/bench` directory.
...
**`iggy-bench` does not start a server.** It only connects, and fails with a connection error when nothing is listening. Start `iggy-server` yourself first:
...
Eight kinds are available; each has a short alias:
