...
QUIC provides built-in TLS encryption and runs the same binary protocol over UDP, built on the quinn library. While not quite as fast as TCP, QUIC provides better security out of the box (TLS is **mandatory**). Defaults:
