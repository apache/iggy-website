...
This node's own client-facing identity, read only while `cluster.enabled = false`. In cluster mode the roster answers the same question per node, so a value here takes no effect and the server says so at startup.
