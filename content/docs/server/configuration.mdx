...
The server reads a single TOML file. A copy of [`core/server/config.toml`](https://github.com/apache/iggy/blob/master/core/server/config.toml) is embedded into the binary and serves as the default configuration, so the server boots with working defaults even without any file on disk. The embedded file is richly commented and is the authoritative reference for the exact version you are running.

A config file does not have to be complete. It is **merged over the embedded defaults**, so a minimal file overrides only what it names:
...
- Unknown keys are **silently ignored**. A typo in a key name means the default silently stays in force, so check spelling when a setting does not seem to take effect. The [relocated keys](#relocated-configuration-keys) below are the exception: they refuse boot.
...
Other sections from older configs (`[message_saver]`, `[system.state]`, `[system.backup]`, `[system.compression]`, `[tcp.socket]`, segment `cache_indexes`, and similar) were removed without a boot check. Like any unknown key they are silently ignored; remove them from your file.
...
The metrics route **requires authentication** like every other read: a missing or invalid bearer credential is rejected with 401. Any authenticated user may scrape; there is no extra RBAC rule. Scrapers present a JWT or a personal access token as the bearer:
...
HTTP TLS is **file-based only**; there is no `self_signed` option for this listener. Both files must exist when enabled.
...
The `self_signed` flag does not mean the same thing everywhere:
