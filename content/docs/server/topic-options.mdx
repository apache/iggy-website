...
Options are key-value pairs sent with `CreateTopic`. Unknown keys are **rejected at the edge, never skipped**: a silently ignored knob would hand the client server defaults without it ever learning. The same mechanism exists for streams and users, but their catalogs are empty today; only topics have option keys.
...
A note on `preallocate_segments`: it reserves exactly `segment_size` of real disk per partition the moment the topic is created (and again as segments rotate). With the default 1 GiB segment size that is 1 GiB per partition up front, which is why it is opt-in and why one create is **capped at 64 GiB** of total reservation.
...
Keys you do not send are resolved to the defaults above by the admitting server and stored as **derived** entries. `GetTopic` returns both blocks, explicit and derived, so the effective value of every knob is always visible, along with who chose it.
...
Updates are **patches**: a key you do not send keeps its current value.
