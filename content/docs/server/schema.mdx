...
Value 0 is reserved and rejected. Values 64..67 are server-internal and refused from clients.
...
- Partition operations and non-replicated operations send the current counter value without advancing it; the server does not track theirs.
...
Replies to non-replicated commands are not result-framed: after `status = 0` the body is the response payload directly.
...
- `Balanced`: `kind = 1`, `length = 0`, no value. Server balances across partitions.
...
`CreateStream`, `UpdateStream`, `CreateTopic`, `UpdateTopic`, `CreateUser`, and `UpdateUser` end with a key-value **options block** that reuses the user-headers TLV encoding. The block runs from its start to the end of the payload (no length prefix; the validator requires exact consumption) and may be empty.
...
After the 48 header bytes: `payload` (`payload_length` bytes), then `user_headers` (`user_headers_length` bytes). Note the payload comes **first**.
...
`metadata_length` counts the bytes from `stream_id` through `messages_count` inclusive, so a reader can skip straight to the batch. The producer leaves `partition_id`, `base_offset`, and `base_timestamp` zero in the batch header; the server stamps them. Every checksum is producer-computed and verified at admission; the reserved regions must be zero.
...
- **Login register. Code: 40.** `Operation::Register`; body is `ClientVersionInfo` + credentials, described [above](#login-register).
- **Login register with PAT. Code: 45.** `Operation::Register`; body is `ClientVersionInfo` + token.
