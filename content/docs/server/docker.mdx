...
Or use `docker compose up` directly from the repository root. Note that this image **differs from the published one**: it sets the `0.0.0.0` listener addresses via `ENV` and keeps its data at `/local_data`.
