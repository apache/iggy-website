...
1. Build the project in release mode (or debug, and update the connectors paths in the config accordingly), and make sure that the plugins specified in `core/connectors/runtime/example_config/connectors/` directory under `path` are available. The configuration must be provided in `toml` format. Note that the example directory enables connectors for ClickHouse, Delta Lake, Apache Doris, Apache Iceberg, and InfluxDB as well - without their backing services (or their compiled plugins) these will be reported with the `Error` status, but they do not block the remaining connectors. Set `enabled = false` in their files to skip them entirely.
...
The same rules apply when it comes to overriding the configuration via environment variables as for the main [Iggy server](/docs/server/configuration).
...
Sources are responsible for producing the messages to the configured stream(s) and topic(s). For example, the Random source connector will generate the random messages that will be then sent to the configured stream and topic.
