...
The target Delta table must already exist at `table_uri`: the connector opens it at startup and fails if it is missing. It does not create tables. Only JSON payloads are supported (`schema = "json"` on the stream); a batch containing another payload type fails.
...
- **GCS**: `gcs_service_account_key` (the service account JSON as a string; the bucket comes from the `gs://` URI).
...
The table schema is captured once at startup and not refreshed, so restart the connector after changing the table schema.
