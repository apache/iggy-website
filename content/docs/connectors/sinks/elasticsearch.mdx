...
Batching is governed by the stream-level `batch_length`: each consume cycle bulk-indexes the batch of messages it receives. The connector performs no retries of its own; failed batches are logged and counted.
...
- Raw payloads are parsed as JSON when possible; otherwise they are wrapped as `{ "data": "<base64>", "data_type": "raw" }`.
