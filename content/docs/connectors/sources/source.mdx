...
Let's implement the example source connector, which will simply generate a random batch of messages at the configured interval, optionally stopping after a total message limit.
...
This particular example additionally uses `humantime`, `rand`, `rmp-serde`, `simd-json`, and `uuid`.
...
Next, a few helpers: `serialize_state()` encodes the state to MessagePack via the `ConnectorState::serialize()` helper, and `generate_messages()` builds a random number of messages (within `messages_range`), each carrying a JSON-serialized `Record` with `payload_size` random characters of text.
...
Now, let's implement the `Source` trait for our `RandomSource` struct. Each `poll()` waits for the configured `interval` to mimic the behavior of a real-world external source, stops producing once `max_count` messages were generated (when it is set), and returns the updated state along with the `ProducedMessages` - the runtime persists that state and hands it back to `new()` after a restart.
