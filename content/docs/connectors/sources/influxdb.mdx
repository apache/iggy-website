...
The InfluxDB source connector polls InfluxDB on an interval and produces the resulting rows as messages into an Iggy stream. It supports both InfluxDB V2 (OSS 2.x / Cloud, Flux queries) and InfluxDB V3 (Core / Enterprise, SQL queries), selected with the `version` option, and tracks its position with a timestamp cursor stored in persistent connector state, so restarts resume where they left off.
...
Omitting `version` defaults to `"v2"` for backward compatibility. The query template must contain the `$cursor` placeholder (enforced at startup); `$limit` is strongly recommended so `batch_size` bounds each query. V3 queries additionally need `OFFSET $offset` unless stuck-batch detection is disabled.
