...
A custom `query` must be a structured object. In TOML that means nested tables (an inline JSON string will not work):
...
When `timestamp_field` is set, the query is additionally filtered to documents whose timestamp is greater than the newest one seen so far, so already-processed documents are not fetched again. Without a `timestamp_field` there is no incremental cursor and each poll returns the first `batch_size` matching documents.
