...
- The `pg_replicate` backend (based on Supabase's ETL framework) is a placeholder and is **not implemented**: selecting `cdc_backend = "pg_replicate"` fails at startup unless the `cdc_pg_replicate` build feature is enabled, and the backend returns an error even when it is.
