...
Transforms mutate, filter, or convert messages as they flow through the connector runtime. They run inside the runtime process, not inside the plugins: for source connectors they are applied after the plugin's messages are decoded and before producing to Iggy, and for sink connectors after consuming from Iggy and before handing the messages to the plugin.
...
- The order of the sections in the file does not determine the execution order - do not rely on one transform seeing the output of another.
...
`field` must not be empty. Messages that are not JSON objects, or that do not contain the field, pass through unchanged.
