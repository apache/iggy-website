---
title: Client Failover
description: Local reads, leader redirection, and retryable errors from the client side
---

A clustered deployment changes what a client can expect from any single node: reads are served locally where possible, writes must reach the right primary, and the SDKs follow the primary as it moves. This page describes that behavior and which SDKs implement it.
...
2. When the connected node is not the leader, the client reconnects to the leader's advertised endpoint for the transport in use.
...
4. Redirects are **capped** (three in the Rust SDK) so a flapping roster cannot bounce the client forever.
