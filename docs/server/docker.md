...
You can easily run the Iggy server with Docker - the official images can be found [here](https://hub.docker.com/r/apache/iggy), use `docker pull apache/iggy:edge` while preparing for 0.9.0, or `apache/iggy:0.9.0` once released.
...
```bash
helm install iggy ./helm/charts/iggy -f my-values.yaml --set server.image.tag=edge
```
