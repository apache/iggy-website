...
These instructions target Iggy server 0.9.0. The container examples use `edge` while the release is being prepared. Use the matching source checkout for local builds.
...
Start an Iggy server with HTTP enabled, as shown in [Getting started](/docs/introduction/getting-started), then run the Web UI as a separate container:

```bash
docker pull apache/iggy-web-ui:edge
docker run -e PUBLIC_IGGY_API_URL=http://localhost:3000 -p 3050:3050 apache/iggy-web-ui:edge
```
...
Topic updates share the HTTP API's [retention update limitations](/docs/server/topic-options). Other topic options require the CLI or HTTP API.
