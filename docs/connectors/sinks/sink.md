...
With the local configuration provider, each sink connector has a TOML file in the directory specified by the main runtime config. The [runtime](/docs/connectors/runtime#configuration-providers) also supports an HTTP configuration provider.
...
Use the matching 0.9.0/edge checkout and broker from the [quick start](/docs/connectors/introduction#quick-start). Its broker must use the credentials below. Create the `connectors` directory, then save these two configuration files there and in the repository root as labeled.
...
Produce JSON messages to this topic, for example with the [Random source](/docs/connectors/sources/random), to see the sink output.
