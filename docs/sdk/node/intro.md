...
These docs target server **0.9.0**. Use the Node SDK's `edge` package with server 0.9.0 or `edge`; the stable Node package 0.8.0 uses the older protocol. The current SDK speaks only the VSR wire protocol and doesn't fall back to older formats.
...
The samples below expect an Iggy server on `127.0.0.1:8090`. See [Getting started](/docs/introduction/getting-started) for the server prerequisites:
...
Examples are available in the [examples/node](https://github.com/apache/iggy/tree/master/examples/node) directory, written in TypeScript. See [Examples](/docs/sdk/node/examples) for the list and how to run them.
